...
async function sendJdRobotNotify(
    text,
) {
    //提供6种通知
    await Promise.all([
        jdRobotNotify(text), //微信server酱
    ]);
}

function jdRobotNotify(text) {
    console.log(`https://server.zjiali.cn/jd/notify/cookieDisabled?nickName=${text}`)
    return new Promise((resolve) => {
        const options = {
            url: `https://server.zjiali.cn/jd/notify/cookieDisabled?nickName=${text}`
        }
        try {
            $.get(options, (err, resp, data) => {
                try {
